...
class HistoryService {
  ...
  Future<Map<int, double>> getPortfolioHistory(List<Asset> assets, int periodDays) async {
    final range = periodDays > 100 ? '1y' 
                : periodDays > 60 ? '3mo' 
                : periodDays > 20 ? '1mo' 
                : periodDays > 6 ? '5d' : '1d';
    ...
    if (needsUsd) {
      ...
      for (final p in usdPoints) usdTryHistory[normalizeTs(p.$1)] = p.$2;
    }
      ...
      for (final p in xauPoints) {
        ...
        if (usdTryHistory.containsKey(dTs)) {
          usdRate = usdTryHistory[dTs]!;
           ...
           final closest = usdTryHistory.entries.lastWhere((e) => e.key <= dTs, orElse: () => usdTryHistory.entries.first);
    ...
    for (final a in assets) {
      ...
      if (a.type == AssetType.hisse || a.type == AssetType.emtia || (a.type == AssetType.doviz && a.ticker == 'USDTRY=X')) {
         if (!tickerNormalizedDaily.containsKey(a.ticker)) {
           final rawPts = await getHistorySafe(a.ticker);
           final map = <int, double>{};
           for (final p in rawPts) map[normalizeTs(p.$1)] = p.$2;
          ...
          try {
             ...
             if (a.type == AssetType.altin) {
                 if (goldHistory.isNotEmpty) {
                    double factor = (a.ticker == 'ALTIN_CEYREK') ? 1.75 : (a.ticker == 'ALTIN_YARIM') ? 3.5 : (a.ticker == 'ALTIN_CUMHURIYET' || a.ticker == 'ALTIN_ATA') ? 7.216 : 1.0;
                 ...
                 if (map.isNotEmpty) {
                    ...
                    if (a.currency == 'USD') {
                       double usdRate = usdTryHistory.isNotEmpty ? _getClosestPrice(usdTryHistory, dayTs, 35.0) : 35.0;
                       price *= usdRate;
                    }
                    assetDayVal = price * a.quantity;
                 } else {
  ...
  double _getClosestPrice(Map<int, double> map, int targetTs, double? fallbackValue) {
