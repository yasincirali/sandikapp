...
| 22 | **`0061_percentile_median.sql`'i koş** | Ana ekrandaki yüzdelik şeridi artık "Getiri sıralaması · medyandan X puan önde/geride" satırı taşıyor; `get_percentile_bucket` DROP + CREATE ile iki sütun daha dönüyor. Migration koşulmadan istemci medyansız çizer (kırılmaz), koşulunca satır kendiliğinden dolar. Havuz, k_min=8 ve Sybil kapısı (0059) aynen. | Actions → **Supabase deploy** → Run workflow (`db push`). Doğrulama: SQL Editor'da `select * from get_percentile_bucket(30);` → dört sütun (`percentile, total_participants, median_roi_pct, my_roi_pct`). |
