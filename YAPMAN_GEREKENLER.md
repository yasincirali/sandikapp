...
| 22 | **`0061_percentile_median.sql`'i koş** | Ana ekrandaki yüzdelik şeridi artık "Getiri sıralaması · medyandan X puan önde/geride" satırı taşıyor; `get_percentile_bucket` DROP + CREATE ile iki sütun daha dönüyor. Migration koşulmadan istemci medyansız çizer (kırılmaz), koşulunca satır kendiliğinden dolar. Havuz, k_min=8 ve Sybil kapısı (0059) aynen. **Durum 2026-09-14:** yerel CLI (2.109.1) bağlı, defter tutarlı, `db push --dry-run` yalnızca 0061'i listeliyor; gerçek `db push` Claude'un izin katmanında durdu. Üç yol: (1) terminalde `supabase db push`; (2) Actions → **Supabase deploy** → Run workflow; (3) `.claude/settings.local.json`'a `Bash(supabase db push*)` izni ekle, Claude koşsun. Doğrulama: SQL Editor'da `select * from get_percentile_bucket(30);` → dört sütun (`percentile, total_participants, median_roi_pct, my_roi_pct`). |
| 23 | **`0062_signal_state_position_key.sql`'i koş + `analyze-signals`'ı yeniden deploy et** | Sinyal de-dup hafızası lot id'sinden pozisyon anahtarına (`pos:<tür>|<TICKER>`) taşındı; temsilci lot silinince gelen fazladan bildirim bitiyor. Migration eski satırları taşır ve kalan lot anahtarlı satır varsa kendini patlatır (`raise exception`). Sıra ÖNEMLİ DEĞİL ama ikisi de gerekli: yalnızca migration koşarsa eski function lot id'siyle yazar (zararsız artık), yalnızca function deploy olursa eski hafıza okunmaz (bir tur fazladan bildirim). | Actions → **Supabase deploy** (`db push` + `functions deploy analyze-signals`) ya da terminalde `supabase db push && supabase functions deploy analyze-signals`. Doğrulama: `select count(*) from signal_state where asset_id not like 'pos:%';` → 0. |
...
| 15 | **Apple ile giriş** | Kod hazır (`SocialAuthService`, entitlement eklendi). Apple tarafı ve Supabase provider ayarı sende. | (a) developer.apple.com → Identifiers → App ID `com.sandik.app` → **Sign In with Apple** yeteneğini aç, provisioning profile'ı yenile. (b) Supabase Dashboard → Authentication → Providers → **Apple**: Enable; "Client IDs" alanına `com.sandik.app` (native akışta Services ID gerekmez, secret key gerekmez). (c) TestFlight'ta dene: ilk girişte ad gelir, sonrakilerde gelmez — profil ilk seferde yazılır. |
