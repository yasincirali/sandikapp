...
/// Sözlük 3.20'de parametre oldu; testler Türkçe sözlükle çağırır.
final _l = AppLocalizationsTr();
String goreliZaman2(DateTime a, DateTime b) => goreliZaman(a, b, _l);
...
void main() {
  ...
  group('eşikler', () {
    test('1 dakikadan yeni → "az önce"', () {
      expect(goreliZaman2(t0, t0), 'az önce');
      expect(goreliZaman2(t0.subtract(const Duration(seconds: 59)), t0),
          'az önce');
    });

    test('dakika aralığı', () {
      expect(goreliZaman2(t0.subtract(const Duration(minutes: 1)), t0),
          '1 dk önce');
      expect(goreliZaman2(t0.subtract(const Duration(minutes: 10)), t0),
          '10 dk önce');
      expect(goreliZaman2(t0.subtract(const Duration(minutes: 59)), t0),
          '59 dk önce');
    });

    test('saat aralığı', () {
      expect(goreliZaman2(t0.subtract(const Duration(minutes: 60)), t0),
          '1 sa önce');
      expect(
          goreliZaman2(t0.subtract(const Duration(hours: 23)), t0), '23 sa önce');
    });

    test('gün aralığı', () {
      expect(
          goreliZaman2(t0.subtract(const Duration(hours: 24)), t0), '1 gün önce');
      expect(
          goreliZaman2(t0.subtract(const Duration(days: 7)), t0), '7 gün önce');
    });
  });
  ...
  test('gelecek zaman negatif sayı BASMAZ', () {
    ...
    expect(goreliZaman2(gelecek, t0), 'az önce');

    final cokGelecek = t0.add(const Duration(hours: 2));
    expect(goreliZaman2(cokGelecek, t0), 'az önce');
  });
  ...
  test('hiçbir çıktı boş ya da "null" değil', () {
    ...
    for (var dk = 0; dk < 60 * 24 * 3; dk += 37) {
      final metin = goreliZaman2(t0.subtract(Duration(minutes: dk)), t0);
