...
L3 (pinning kararı). **Tamamlanan:** Faz 0 (tümü), Faz 1 (tümü), Faz 2: 2.1–2.11, 2.13 (kısmi);
...
| 3.2 Baz para birimi (USD/EUR/altın) | 🟡 Çekirdek 2026-09-14: Ayarlar › Görünüm'de TRY/USD/EUR/gram altın seçici (`baseCurrencyIndexProvider`, kişiye özel); `BazPara`/`ParaBicimi` (`utils/money_format.dart`) TRY tutarını bugünkü kurla çevirip yazar, kur yoksa ₺'ye düşer. Bağlanan yüzeyler: ana ekran hero + kişi kartları, Portföy (donut, varlık kartı, detay paneli), Performans (özet, grafik ekseni/tooltip, tür dökümü, dönem özeti, katkı kartı). **Kalan (₺ sabit):** tekil varlık ekranı, takip listesi, hareket satırları (geçmiş tutar), bildirim/özet metinleri (daily_summary, recap, milestone), ana ekran widget'ı ve Live Activity. Cihazda görsel doğrulama sende. |
...
| 2.12 / 2.14 | Görsel doğrulama isteyen UI kalemleri (segment kontrolleri, hero yeniden tasarımı); cihazsız yapılmadı. 2.10 ve 2.13 `Size.zero` 2026-09-14'te kapandı. |
...
| 2.12 | ⏳ | |
| 2.13 | 🟡 kısmi | `ZoomableChart.semanticLabel`; sparkline bilinçli sessiz. 2026-09-14: `minimumSize: Size.zero` butonlar 44pt'ye çıkarıldı (`SandikTouch.min`, `touch_target_size_test` ratchet), tür/döviz çipleri ve ayarlar seçicileri `Semantics(button, selected)`. **Kalan:** kompakt yüzeylerde ok glifi. |
| 2.14 | ⏳ | |
