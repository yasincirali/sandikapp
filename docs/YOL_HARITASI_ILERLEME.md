...
Faz 3: 3.3, 3.4, 3.5, 3.6, 3.7, 3.8(kısmi), 3.15, 3.17(kısmi), 3.19(kısmi) tamam. Sıradaki: 3.2 baz para birimi, 3.11 sessiz saatler (sunucu), 3.16 integration test; 3.1 (Apple/Google — geliştirici hesabı ister), 3.9/3.10 büyük refactor'lar.
...
| 3.17 | 🟡 kısmi | `test/deposit_and_tefas_model_test.dart`: mevduat encode/decode, basit/bileşik faiz, stopaj, vade; `TefasFund` JSON round-trip. `remote_push_service`, `data_export_service`, `price_alert_service` hâlâ testsiz — üçü de Supabase/Firebase istemcisine bağlı, arayüz soyutlaması ister. |
